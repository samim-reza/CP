...
ull exponial_chk(ull k, ull limit) {
...
ull exponial(ull n, ull m) {
if (m == 1) return 0;
if (n == 1) return 1 % m;
ull ph = phi(m);
ull exp_mod = exponial(n-1, ph);
ull chk = exponial_chk(n-1, ph);
bool is_large = (chk >= ph);
ull exp = exp_mod + (is_large ? ph : 0ULL);
return BigMod(n % m, exp, m);
}
